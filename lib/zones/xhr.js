...
function browserZone(data){
	...
	var tingApiAttributes = ['/json/', '/api/tmw/'];

	var matches = function(request, url, body) {

		return (removePrefixForTing(request.url) === url ) &&
			(!body || request.data === body);
	};

	var removePrefixForTing = function (url) {
		let parsedUrl;
		for (var i = 0; i < tingApiAttributes.length; i++) {
			let stringIndex = url.search(tingApiAttributes[i]);
			if (stringIndex !== -1) {
				parsedUrl = url.substring(stringIndex, url.length);
				break;
			}
		}
		return parsedUrl;
